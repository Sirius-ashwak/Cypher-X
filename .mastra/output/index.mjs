...
import { createTool, Agent, Mastra, createWorkflow, createStep, generateEmptyFromSchema, Telemetry } from '@mastra/core';
import { z, ZodFirstPartyTypeKind } from 'zod';
import winston from 'winston';
import { mkdirSync, createReadStream, lstatSync } from 'fs';
...
logger$1.info("Project Cypher logger initialized", {
  logLevel: process.env.LOG_LEVEL || "info",
  timestamp: (/* @__PURE__ */ new Date()).toISOString()
});

const tradingTool = createTool({
  id: "tradingTool",
  ...
  execute: async (context) => {
    logger$1.info("Trading tool executed", context);
    return {
      success: true,
...
logger$1.info("CypherAgent initialized successfully", {
...
const analyzeMarketStep = createStep({
  ...
  execute: async (_context) => {
    logger$1.info("Analyzing market");
